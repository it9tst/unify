...
window.addEventListener("load", function(){
	Player = (function(){
		function Player(){
			...
			var _link=urlSite+"json.php?request=player";
			var _linkAlbum= urlSite+"images/albums/";
			var _idSong=-1;
			var _idAlbum=-1;
			var _idArtist=-1;
			var _idPlaylist=-1;
			...
			function _load(id){
				...
				xhttp.open("GET",_link+"&action=info&id="+id+"&phpsess="+window.localStorage.getItem("phpsessid"), true);
				xhttp.send();
				return true;
			}
			...
			function _loadPlaylist(id){
				...
				xhttp.open("GET",_link+"&action=info&type=playlist&id="+id+"&phpsess="+window.localStorage.getItem("phpsessid"), true);
				xhttp.send();
				return true;
			}
			function _loadAlbum(id){
				...
				xhttp.open("GET",_link+"&action=info&type=album&id="+id+"&phpsess="+window.localStorage.getItem("phpsessid"), true);
				xhttp.send();
				return true;
			}
			function _loadArtist(id){
				...
				xhttp.open("GET",_link+"&action=info&type=artist&id="+id+"&phpsess="+window.localStorage.getItem("phpsessid"), true);
				xhttp.send();
				return true;
			}
			...
			function _parseResponse(json){
				...
				_playerAudio.src=urlSite+"json.php?request=player&idSong="+json.Id+"&phpsess="+window.localStorage.getItem("phpsessid");
			...
			function _createQueueElement(titolo, artista, image){
				...
				li.innerHTML='<img src="'+urlSite+'images/albums/'+image+'"><span class="music">'+titolo+'</span><span class="artist">'+artista.replace(",", ", ")+'</span><span style="flex-grow:1"></span><i class="fas fa-play" style="color:#a3a3a3"></i>';
				return li;
			}
