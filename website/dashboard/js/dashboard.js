...
function loadCenterTemplateFunc(dataLink, dataAddict, dataExec, dataParams, pushState=true){
	var link="/json.php?request=dashboard&action=loadtemplate&datalink=";
	if(dataLink){
		var state={};
		state.link=dataLink;
		link+=dataLink;
...
function _request(func, action, params=Array(), request="dashboard"){
	...
	_xhttp.open("POST", "/json.php?request="+request+"&action="+action, true);
...
function searchFunction(){
	...
	if(typeof key!="undefined" && key!=""){
		...
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				dashCenter.innerHTML=this.responseText;
		...
		xhttp.open("GET", "/json.php?request=dashboard&action=search&key="+key, true);
		xhttp.send();
	}else{
...
function cliccaSalva(){ //gestisce il click del pulsante salva
	if(!checkPass(input[3].value))
		...
		if(!toSend){
			...
			var link="/json.php?request=dashboard&action=updateAccountSettings";
			var sessid=window.localStorage.getItem('PHPSESSID');
			link+="&sessid="+sessid
			$.ajax({
				url: "/json.php?request=dashboard&action=updateAccountSettings",
				data: formData,
				cache: false,
				contentType: false,
				processData: false,
